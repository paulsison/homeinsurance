...
public class Scenario_04 {	
	 ...
	 * to retrieve a previously generated quote. */
	...
	public void RetrieveQuoteTest() throws IOException {
		...
		// Expected test data
		double dwellingCoverage = 283500.0;
		...
		//We retrieve an existing quote
		driver.findElement(By.cssSelector("body > form:nth-child(5) > input[type=\"submit\"]")).click();

		//From the quote page we verify the expected dwelling coverage as
		//for the first quote and only quote for this test user.
		assertThat(driver.getTitle(), equalTo("My Quotes"));
		takeScreenShot(driver);
				//We may need a more dynamic selector for varying quantity of quotes in our test
		WebElement receivedQuote = driver.findElement(By.cssSelector("body > table > tbody > tr:nth-child(2) > td:nth-child(3)"));
		String quoteString = receivedQuote.getText();
		assertThat(Double.parseDouble(quoteString), equalTo(dwellingCoverage));
